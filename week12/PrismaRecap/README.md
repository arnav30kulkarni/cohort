...
you can add relationships by using @relations in  data model in schema.prisma
...
model User{
  ...
  lastname  String
  email String @unique
  todos Todo[] // Every user can have multiple todos
}
...
model Todo {
  ...
  userId Int
  user User @relation(fields: [userId] , references: [id]) // Every todo has a relation with user on the user_id with ref to id
}
```
